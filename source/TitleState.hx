...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		FlxG.save.bind('funkin', 'therealferret');
		...
		if(ClientPrefs.checkForUpdates && !closedState) {
			...
			{
				newF6KRVersion = data;
				f6krUpdateVersion = data.split('\n')[0].trim();
				var curVersion:String = MainMenuState.f6krVersion.trim();
				trace('f6kr version online: ' + f6krUpdateVersion + ', your f6kr version: ' + curVersion);
				if(f6krUpdateVersion != curVersion) {
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (mustUpdate || mustUpdateF6KR) {
						MusicBeatState.switchState(new OutdatedState(newVersion, mustUpdateF6KR, newF6KRVersion));
					} else {
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				// credTextShit.screenCenter();
				case 14:
					addMoreText('Ferret\'s');
				// credTextShit.visible = true;
				case 15:
					addMoreText('6 Key');
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Recharts');
