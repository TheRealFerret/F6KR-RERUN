...
class TaskSong extends FlxSpriteGroup
{
    ...
    public function new(_x:Float, _y:Float, _song:String, ?_numberThing:Int = -1) {
        ...
        var text3 = new FlxText(0, 60, 0, "", fontSize);
        text3.setFormat(Paths.font("arial.ttf"), fontSize, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        ...
        size = text3.fieldWidth;
        
        var bg = new FlxSprite(fontSize/-2, fontSize/-2).makeGraphic(Math.floor(size + fontSize), Std.int(text.height + text2.height + text3.height + 15), FlxColor.WHITE);
        bg.height = text.height + text2.height + text3.height;
        bg.alpha = 0.47;
